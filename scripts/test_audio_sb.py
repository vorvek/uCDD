...
def main():
    ...
    parser.add_argument('--cpu', choices=('386', '586'), default='386')
    ...
    files['SBTEST.CUE'] = (b'FILE "SBTEST.BIN" BINARY\r\nTRACK 01 MODE1/2352\r\nINDEX 01 00:00:00\r\n'
                           b'TRACK 02 AUDIO\r\nINDEX 01 00:00:22\r\n')
    files['FDCONFIG.SYS'] = files['FDCONFIG.SYS'].replace(b'DOS=LOW', b'DOS=HIGH,UMB').replace(
    ...
    for output in args.output or ('sb16', 'sbpro', 'wss', 'sb'):
        ...
        for client in ('SBIRQ', 'SBRM'):
            commands = ['@ECHO OFF', 'LH UCDD -install', 'IF ERRORLEVEL 1 GOTO FAIL',
                        ...
                        'IF ERRORLEVEL 1 GOTO FAIL', 'RESSTATE H', 'IF ERRORLEVEL 1 GOTO FAIL',
                        'PASS', ':FAIL', 'FAIL']
            if args.irq == 5:
            ...
            result = subprocess.run([str(capture), str(image), str(wav)], capture_output=True,
                                    text=True, timeout=90, env=dict(os.environ, UCDD_TEST_CPU=args.cpu,
                                    UCDD_TEST_STEPS='90000', UCDD_TEST_OUTPUT=output))
            (run/'test.log').write_text(result.stdout+result.stderr)
