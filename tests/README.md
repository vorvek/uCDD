...
python scripts/test_audio_buffer_relocation.py --izarra-source D:\dev\IzarraVM
python scripts/test_audio_resident.py --izarra-source D:\dev\IzarraVM --quake-dir C:\games\quake --quake-bin C:\images\quake.bin --load-high --own-host --dynarec
python scripts/test_audio_resident.py --izarra-source D:\dev\IzarraVM --quake-dir C:\games\quake --quake-bin C:\images\quake.bin --load-high --own-host --dynarec --runs 1 --fragment-umb
python scripts/test_audio_resident.py --izarra-source D:\dev\IzarraVM --quake-dir C:\games\quake --quake-bin C:\images\quake.bin --load-high --own-host --dynarec --runs 1 --fragment-umb --ems
...
The interpreted 386 boot test forces a wrong physical I/O address. It checks failed-install rollback of interrupt vectors, PIC mask, saved mixer registers, DOS allocation policy, and largest available DOS block. The driver header and private port interface must be absent afterward. It also rejects `-ems` when EMS is unavailable, then installs normally. A corrected configuration must install successfully. F2 in the configurator must produce no speaker-test tone while resident audio owns the card; F10 must preserve the settings. Reports are under `.local/audio/resident-boot-own-host/`.

The interpreted 386 buffer-relocation test leaves 29 KiB and 12 KiB upper-memory blocks. The transient installer cannot fit, but the resident driver and CD mix buffer can move high. The test reads a resident snapshot and requires each XMS move descriptor to address its current buffer. This catches a stale address caused by a BP-relative store using SS after the driver moves to a different DS. Reports are under `.local/audio/buffer-relocation/`. The game runner's `--fragment-umb` option applies the same memory layout to the full mixed-audio checks.

The 586 game test installs before mounting an image, checks duplicate installation and rejected mount commands, copies and verifies RESOURCE.1, then starts Quake directly twice with an unmount/remount between runs. It uses the interpreter by default. `--dynarec` selects the automatic execution backend for a faster local regression run and requires the internal host. Each run must finish, leave no reported audio fault, and pass separate checks of the complete 12-second music excerpt and centered game sound. Each capture must also reject four deliberate corruptions. The phase markers delimit the two game runs in the capture. `--mscdex` replaces SHSUCDX with a caller-supplied MSCDEX executable; MSCDEX 2.25 passes the one-run data and mixed-audio case. Original game files, images, and redirector binaries are read-only inputs.

With `--load-high`, the one-unit driver/mixer, private host stack, and real-mode host gateway must be above conventional memory when suitable UMBs are available. The mix and work buffers also move high when they fit. The DMA ring and its allocation must remain below A0000h. In the standard Jemm card test, the high blocks use 26,784, 2,048, 6,656, 2,128, and 6,144 bytes. The 8,192-byte DMA allocation and its 16-byte MCB header are the only conventional cost. The default queue uses 512 KiB of XMS, and the protected host uses about 63 KiB of XMS. Reports are under `.local/audio/resident-high-own-host/`, or `resident-low-own-host/` without `--load-high`. These checks establish the tested Jemm/internal-host path. Tomb Raider covers timer refills without frequent CD status polling. `--ems` selects the EMS queue for the same complete Quake waveform checks, not only an allocation test.

### Error-path and shared-resource regressions

```powershell
python scripts/test_audio_audit.py --izarra-source D:\dev\IzarraVM
python scripts/test_memory_ownership.py --capture <capture.exe> --base-image .local/host/monitor-external.img --output .local/ownership/unmap --case unmap
```

The audio runner uses the 386 interpreter. It checks unrelated DMA mask/mode forwarding, physical and virtual flip-flop resets, global mask handling, and exclusion of the physical output channel. The EMM386 snapshot test gives every input port a distinct value and selects physical DMA3/6 while requiring virtual DMA1/5. The DSP test holds the busy bit after a successful reset, before the first or a later command, then checks command suppression, failed startup, IRQ restoration, and reset/retry.

The EMS tests insert an interrupt probe at each shared descriptor store and at flag restoration. They require unchanged queue bytes and return status, and preservation of both enabled and disabled caller IF states. EMS manager replies and physical I/O are simulated in these focused tests; the generated routines come from the current source. Full Quake EMS streaming is checked separately above. CD range tests execute the actual request handler and DOS file reads with a simulated XMS mover, accepting audio-to-audio boundaries and rejecting entry into data tracks in HSG and MSF requests.

The ownership runner takes a previously built capture executable and the external-host base disk from `test_host.py`. Its `unmap` case checks zero and 0FFFh after a valid physical map/free, low-memory integrity, and exact VCPI recovery across two clients. Cases `detach-high`, `detach-low`, and `detach-clean` inject post-trap startup and removal failures into temporary source overlays. They check actual registered callbacks, retained code/buffer/queue ownership, stopped physical output, and successful-detach rollback. Production builds contain no fault-injection hooks. Give each case a separate output directory; reports record exact binaries and source hashes.

For an end-to-end alternate-resource check, run `test_audio_sb.py --cpu 486 --output sb16 --dma8 3 --dma16 6 --himem <HIMEM.SYS> --emm386 <EMM386.EXE> --izarra-source <checkout>`. The game-facing channels remain DMA1/5. Both the protected and real-mode clients must pass the mixed-waveform checks. The initial snapshot regression above uses the 386 interpreter; the streaming client uses the faster interpreted 486 because the 386 fixture misses refill deadlines.
...
The internal host is linked into the resident UCDD.EXE build. `test_host_audit.py` checks the external 32-bit DPMI client interface in 22 cases: core services, descriptor and buffer validation, allocation exhaustion, virtual PIC routing, real-mode IRQ forwarding, nested IRQ/callback stacks, revoked callbacks, callback faults and exits, private entry rejection, malformed return frames, IVT cleanup, and allocation rollback. Each case runs its client twice and requires exact VCPI free-page recovery. It records source and executable hashes, guest logs, and memory dumps under `.local/host/audit/`. Use `--case` to select a case.
...
The `real-irq` case checks IRQ delivery during real-mode services, deferred delivery with virtual interrupts disabled, and chaining to the original BIOS handler. `real-irq-nested` adds a second real-mode service inside the forwarded handler and checks its IRQ-stack canary. The forwarding service uses an 8 KiB protected stack and a 2 KiB real-mode stack for up to four levels. The latter is allocated UMB-first for the active client and released at exit.

The `callback-irq-free` case verifies that releasing a callback restores the real IRQ vector behind the forwarding hook.
...
python scripts/test_audio_cards.py --izarra-source D:\dev\IzarraVM --output sb16 --load-high --umb-kib 29 --umb-extra-kib 12
python scripts/test_audio_cards.py --izarra-source D:\dev\IzarraVM --output sb16 --load-high --umb-kib 29 --umb-extra-kib 12 --ems
python scripts/test_audio_cards.py --izarra-source D:\dev\IzarraVM --output sb16 --load-high --umb-kib 29 --umb-extra-kib 12 --without-ems-code
```
...
A protected client then runs twice through the internal host, requests WSS IRQ 11, and checks disabled codec interrupts, PIC masking, and resumed delivery. This runs with each physical output. Direct real-mode WSS interrupt delivery is not implemented. The lifecycle wrapper compares DOS allocation state, vectors, PIC mask, and mixer state. The high-memory cases check every resident DOS block and read conventional sizes from DOS memory control blocks. SB16/WSS use 8 KiB; SB Pro uses 2 KiB. The 29 KiB plus 12 KiB fixture leaves 14,592 payload bytes conventional with SB16: 8,192 bytes for DMA, 6,144 bytes for the work buffer, and a 256-byte owner stub. Their three MCB headers bring the conventional cost to 14,640 bytes. Smaller gaps can force the page-aligned host gateway and its stack low; the largest UMB size alone does not determine placement. `--ems` puts the queue in 32 EMS pages. `--without-ems-code` builds the XMS-only control; the measured installed driver/mixer difference is 384 bytes. Reports are under `.local/audio/cards-<output>-high/`.
